...
if bashio::config.has_value 'vcontrol_host'; then
    ...
    bashio::config.true 'debug' && bashio::log.info "vcontrol_host not set in configuration. Defaulting to 'localhost'." || true
...
if bashio::config.has_value 'vcontrol_port'; then
    ...
    bashio::config.true 'debug' && bashio::log.info "vcontrol_port not set in configuration. Defaulting to '3002'." || true
